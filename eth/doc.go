...
// 本模块路径：github.com/godaddy-x/wallet-adapter-eth。
//
// 实现 ChainAdapter 接口，支持原生币与 ERC20 建单、EIP-155 签名、原始交易广播，可与 MPC 签名服务配合。
// 内部按职责分包：config（EVM WalletConfig）、rpc、decoder、manager、models、util；通用 Configer 接口定义来自 github.com/godaddy-x/wallet-adapter/config。
// 对外仅暴露本包：EthAdapter、NewEthAdapter、NewAdapter。
//
// 使用方式：
//
//  1. 一行启动（从 JSON 内容创建并注册）：
...
//     a.LoadAssetsConfig(c)  // c 为 github.com/godaddy-x/wallet-adapter/config.Configer 或 map[string]string
//
